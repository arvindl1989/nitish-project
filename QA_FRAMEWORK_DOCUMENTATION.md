...
### 4.1 Master Engine Blueprint ([`bookmarklet.js`](file:///c:/git-repo/QA/aem-qa-tool/bookmarklet.js))
...
1. **[`build-cloud-bookmarklet.js`](file:///c:/git-repo/QA/aem-qa-tool/build-cloud-bookmarklet.js)**:
   - Encodes bookmarklet to auto-detect Vercel server origin dynamically or send to `/api/report`.
   - Output: [`AEM_QA_BOOKMARK.txt`](file:///c:/git-repo/QA/aem-qa-tool/AEM_QA_BOOKMARK.txt) & [`AEM_QA_BOOKMARK_CLOUD.txt`](file:///c:/git-repo/QA/aem-qa-tool/AEM_QA_BOOKMARK_CLOUD.txt).

2. **[`build-bookmarklet.js`](file:///c:/git-repo/QA/aem-qa-tool/build-bookmarklet.js)**:
   - Configured for local development targeting `http://localhost:3500/report`.
   - Output: [`AEM_QA_BOOKMARK_LOCAL.txt`](file:///c:/git-repo/QA/aem-qa-tool/AEM_QA_BOOKMARK_LOCAL.txt).

3. **[`build-standalone.js`](file:///c:/git-repo/QA/aem-qa-tool/build-standalone.js)**:
   - Generates an offline self-contained bookmarklet payload that renders UI without requiring external server endpoints.
   - Output: [`AEM_QA_BOOKMARK_STANDALONE.txt`](file:///c:/git-repo/QA/aem-qa-tool/AEM_QA_BOOKMARK_STANDALONE.txt).
...
The central dashboard is served from `public/` ([`public/index.html`](file:///c:/git-repo/QA/public/index.html), [`public/app.js`](file:///c:/git-repo/QA/public/app.js), [`public/style.css`](file:///c:/git-repo/QA/public/style.css)).
...
Modify the `CONFIG.selectors` dictionary in [`bookmarklet.js`](file:///c:/git-repo/QA/aem-qa-tool/bookmarklet.js):
...
> For sprint presentation walkthroughs, see [`SPRINT_DEMO_PRESENTATION_GUIDE.md`](file:///c:/git-repo/QA/SPRINT_DEMO_PRESENTATION_GUIDE.md).
